...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    try
    {
        ...
        let identifierName = 'profit_center'
        let id = 0
        let columnName = ['code']
        let columnValue = 
        {
        ...
        if(uniqueCheck == 0)
        {
            ...
            isActive = 1
            let saveProfitCenter = await db.saveProfitCenter(uuid, code, description, createdOn, createdById, isActive)
            if(saveProfitCenter.affectedRows > 0)
            {
                let returnUuid = await db.getReturnUuid(saveProfitCenter.insertId)
                res.status(200)
                return res.json({
                    "status_code" : 200,
                    "message"     : "success",
            ...
            else
            {
                ...
                return res.json({
                    "status_code" : 500,
                    "message"     : "Profit Center not saved",
        ...
        else if(uniqueCheck == 1)
        {
            ...
            return res.json({
                "status_code" : 400,
                "message"     : "Profit Center code Already Exist",
                "status_name" : getCode.getStatus(400)
            });
        }
        else
