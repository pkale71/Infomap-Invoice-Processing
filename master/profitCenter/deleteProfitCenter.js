...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    try
    {
        ...
        if(checkUsed[0].isExist == 0)
        {
            ...
            let deleteProfitCenter = await db.deleteProfitCenter(uuid, isActive, modifyOn, modifyById)
            if(deleteProfitCenter.affectedRows > 0)
            {
                res.status(200)
                return res.json({
                    "status_code" : 200,
            ...
            else
            {
                ...
                return res.json({
                    "status_code" : 500,
                    "message"     : "Profit Center not deleted",
        ...
        else
        {
            ...
            return res.json({
                "status_code" : 400,
                "message"     : "Profit Center is already in use",
                "status_name" : getCode.getStatus(400)
            });
        }
    }
