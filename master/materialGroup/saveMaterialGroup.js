...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    try
    {
        ...
        let identifierName = 'material_group'
        let id = 0
        let columnName = ['code']
        let columnValue = 
        {
        ...
        if(uniqueCheck == 0)
        {
            ...
            isActive = 1
            let saveMaterialGroup = await db.saveMaterialGroup(uuid, code, description, createdOn, createdById, isActive)
            if(saveMaterialGroup.affectedRows > 0)
            {
                let returnUuid = await db.getReturnUuid(saveMaterialGroup.insertId)
                res.status(200)
                return res.json({
                    "status_code" : 200,
                    "message"     : "success",
            ...
            else
            {
                ...
                return res.json({
                    "status_code" : 500,
                    "message"     : "Material Group not saved",
        ...
        else if(uniqueCheck == 1)
        {
            ...
            return res.json({
                "status_code" : 400,
                "message"     : "Material Group code Already Exist",
                "status_name" : getCode.getStatus(400)
            });
        }
        else
